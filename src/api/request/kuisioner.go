...
type (
	Kuisioner struct {
		...
		F5b   string  `json:"f5b"`
		F5c   int8    `json:"f5c" validate:"min=0,max=4"`
		F5d   int8    `json:"f5d" validate:"min=0,max=3"`
		F18a  int8    `json:"f18a" validate:"min=0,max=2"`
...
func (r *EditKuisioner) MapRequest() *model.Kuisioner {
	parseBool := func(v int8) bool {
		return v == 1
	}
