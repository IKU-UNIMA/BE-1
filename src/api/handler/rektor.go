...
const getRektorQuery = "SELECT rektor.id, nama, nip, akun.email, bagian FROM rektor JOIN akun ON rektor.id = rektor.id"
...
func GetRektorByIdHandler(c echo.Context) error {
	...
	condition := getRektorQuery + fmt.Sprintf(" WHERE rektor.id = %d", id)
